...
void draw(){
  ...
  float phi1 = float(frameCount)/200;
  float lambda0 = float(frameCount)/40;
  ...
  for(float x = -outSize; x < outSize; x+=tileS){
      for(float y = -outSize; y < outSize; y+=tileS){
        PVector p2 = unAzimuthalProjection(projSize, x, y, phi1, lambda0);//-half_pi < phi(x) < half_pi, -pi < lamda(y) < pi
